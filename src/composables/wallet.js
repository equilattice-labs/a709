...
export const publicProvider = new JsonRpcProvider(network.rpc, network.id, {
  staticNetwork: true,
});
...
export function discoverWallets() {
  ...
  window.addEventListener("eip6963:announceProvider", (event) => {
    ...
    if (provider && info && !wallet.providers.some((p) => p.id === info.uuid))
      wallet.providers.push({
        id: info.uuid,
        name: info.name,
  ...
  if (window.ethereum && !wallet.providers.length)
    wallet.providers.push({
      id: "injected",
...
export function readableError(error) {
  ...
  if (error?.code === "INSUFFICIENT_FUNDS")
    return "You need testnet ETH for gas. Get some from the official faucet.";
  if (error?.code === -32002)
    return "A request is already open. Check your wallet to continue.";
  return (
    error?.shortMessage ||
    error?.reason ||
    error?.message ||
...
export async function connectWallet(choice) {
  ...
  try {
    ...
    if (!selected)
      throw new Error(
        "No browser wallet detected. Install an EVM wallet or open this site in your wallet’s browser.",
      );
    ...
    if (!accounts[0])
      throw new Error("No wallet account was selected. Please try again.");
    if (activeProvider?.removeListener) {
      activeProvider.removeListener("accountsChanged", activeListener);
      activeProvider.removeListener("chainChanged", chainListener);
    }
    activeProvider = candidate;
    wallet.selected = selected.name;
    wallet.address = accounts[0] || "";
    wallet.chainId = chainId;
    wrappedProvider = new BrowserProvider(activeProvider, "any");
    activeListener = (accounts) => {
      wallet.address = accounts[0] || "";
      if (!wallet.address) wallet.selected = null;
    };
    chainListener = (chainId) => {
      wallet.chainId = Number(chainId);
      wrappedProvider = new BrowserProvider(activeProvider, "any");
    };
    activeProvider.on?.("accountsChanged", activeListener);
    activeProvider.on?.("chainChanged", chainListener);
    wallet.show = false;
  } catch (error) {
    wallet.error = readableError(error);
  } finally {
...
export function disconnectWallet() {
  ...
  wrappedProvider = undefined;
  wallet.show = false;
}
export async function ensureNetwork() {
  if (!wallet.address || !activeProvider)
  ...
  if (wallet.chainId !== network.id)
    throw new Error("Please switch to Robinhood Chain Testnet in your wallet.");
  wrappedProvider = new BrowserProvider(activeProvider, "any");
...
export async function tokenInfo(address) {
  ...
  if (Number(decimals) > 36)
    throw new Error(
      "Tokens with more than 36 decimals are not supported in this interface.",
    );
  return { address, symbol, decimals: Number(decimals) };
}
export const displayAmount = (amount, decimals = 6) =>
  Number(formatUnits(amount || 0n, decimals)).toLocaleString("en-US", {
