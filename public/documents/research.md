# Vestlyr — product, network and design research
...
The useful product pattern is a single contract primitive viewed through four user intentions. Vestlyr adopts that functional model, with its own implementation, wording, visual identity and brand assets. The reference's source, logos, artwork, claimed audits and customer proof do not constitute evidence about Vestlyr.

The reference contains inconsistent cancellation descriptions: one section says creator cancellation before the start; an FAQ describes cancellation after vesting with a split refund. Vestlyr resolves this explicitly: **only a schedule configured as cancelable can be canceled by its creator, and only before its start**. After the start, the creator cannot revoke it. A Vestlyr cliff gates access to the amount accrued from the start; it is not a separately configurable initial cliff lump sum. A user must see these rules before funding. Any difference from the reference is intentional and should be documented in the product guide.
...
| Mainnet and testnet exist | [Connecting](https://docs.robinhood.com/chain/connecting) lists mainnet `4663` and testnet `46630`. | Describe **Vestlyr as a testnet release**. Do not say Robinhood Chain itself is only a testnet. |
...
| Site | Observed content and hierarchy | Observed visual / interaction pattern | Decision for Vestlyr |
| --- | --- | --- | --- |
...
Recommended composition: **Sablier's narrative sequence + Lido's editorial spacing + Uniswap's task focus**, with Jupiter's persistent state indicators inside the workspace. Vestlyr should use an original, coherent identity built around time and release layers, with legible typography, a meaningful schedule visualization, responsive layouts and reduced-motion support. A schedule preview should be useful even without a wallet; an illustrative preview must be visibly labeled.
...
| Superfluid | [Homepage](https://www.superfluid.org/) positions continuous money streaming as infrastructure for recurring onchain payments, rewards and composable applications. | Vestlyr's finite, fully funded schedules are a narrower product. Avoid claiming equivalence to an open-ended streaming balance protocol. |
| Unlockle reference | [Reference site](https://unlocklexyz.vercel.app/) combines four modes on Robinhood Chain Testnet. Its copy includes audit and fee-control claims. | It supplies a workflow benchmark, not differentiation or a security audit for this build. Vestlyr needs independently tested behavior and a different operational proposition. |

Sablier, Lido and Superfluid display adoption/financial counters. Those numbers were not verified against onchain datasets and are deliberately excluded from the market-sizing model. No customer or partner of those products is represented as a Vestlyr customer or partner.
